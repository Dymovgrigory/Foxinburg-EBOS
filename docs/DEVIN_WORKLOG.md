...
### Сессия 2026-06-27 — Лендинг: герой «для кого система в целом» (4 аудитории сверху) ✅ смёржен (PR #19)

**Задача (от владельца).** «Верхняя часть лендинга должна быть построена так, чтобы там была
информация сразу для кого эта система в целом! а ниже подробности добавь побольше информации».

**Что сделано (только `frontend/src/pages/LandingPage.tsx`).**
- **Герой переделан** с World-ориентированного на обзор всей экосистемы: бейдж «Единая
  образовательная экосистема», `h1 "FOXINBURG — образование, которое вдохновляет"`, описание
  (World + EBOS + Академия), 2 CTA (Попробовать бесплатно → регистрация на `/world`; Запросить
  демо для школы → скролл к форме `#demo`), trust-чипы.
- **4 кликабельные карточки аудиторий прямо в первом экране** (сразу видно «для кого»):
  ученики/родители → Foxinburg World; владельцы школ → EBOS; преподаватели/админы школы →
  Академия+кабинет; самостоятельные специалисты → только Академия. Каждая карточка — со своим
  CTA-onClick (`openWorldSignup` / `scrollTo('demo')` / `openLogin` / `openAcademySignup`).
- **Подробности ниже расширены:** в массиве `segments` добавлен короткий `summary` (для карточек
  героя) и развёрнутые `points` (по 5 пунктов вместо 3) — детальный разбор «кому подходит»
  остался секцией `#segments`. В секцию `#world` добавлен блок «6 миров — путь от новичка до
  эксперта»: маскот + карта миров A1→C2 (переиспользован массив `WORLDS`, ранее живший в герое).
- Секции World / «Как начать» / Цены / EBOS / Школам (форма демо) / FAQ / футер сохранены —
  переорганизация, без удаления. Бэкенд, цены, подписка не менялись. localStorage не используется.

**Проверки.** `npx tsc -b` — чисто; `eslint src/pages/LandingPage.tsx` — 0 ошибок; `npm run build`
— успешно. Локально в браузере: первый экран показывает все 4 аудитории без скролла, карта миров
и расширенные секции рендерятся, ошибок в консоли нет.

**Статус.** PR #19 смёржен в `main` → выкатывается на foxinburg.ru автодеплоем.

**Что дальше по World (на выбор владельца, ещё не начато):**
- Наполнить миры реальным контентом A1 (уроки/тесты/ДЗ) вместо заглушечной структуры.
- Довести оплату подписки: прод-ключи Tinkoff (Rebill) + страницы успеха/неуспеха оплаты.
- (Отложено владельцем) мобильное приложение через Capacitor → RuStore (Android); iOS — только PWA/App Store.

---

### Сессия 2026-06-27 — Автопровижининг миров при старте бэкенда ✅ смёржен (PR #18)
